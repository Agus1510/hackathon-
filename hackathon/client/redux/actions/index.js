...
export const register = createAsyncThunk(
  ...
  async (userData, thunkAPI) => {
    try {
      const metaData = await axios.post("/malmo/auth/register", userData);
...
export const login = createAsyncThunk(
  ...
  async (post, thunkAPI) => {
    try {
      const metaData = await axios.post("/malmo/auth/login", post);
      thunkAPI.dispatch(setValidateUser(metaData.data.user));
  ...
  async (obj, thunkAPI) => {
    try {
      ...
      let metaData = await axios.get(
        `/malmo/usersprivate/username?username=${obj.input}`,
        config
      );
  ...
  async (obj, thunkAPI) => {
    ...
    try {
      const metaData = await axios.put(
        `/malmo/productsprivate/${obj.id}/edit`,
...
export const editUsername = createAsyncThunk(
  ...
  async (obj, thunkAPI) => {
    ...
    try {
      const metaData = await axios.put(
        `/malmo/usersprivate/${obj.id}/profile`,
...
export const editPassword = createAsyncThunk(
  ...
  async (email) => {
    try {
      const metaData = await axios.put(`/malmo/auth/forgotPassword`, {
        email: email,
      });
      return metaData.data;
    } catch (err) {
  ...
  async (obj, thunkAPI) => {
    try {
      ...
      const metaData = await axios(
        `/malmo/usersprivate/?limit=8&page=${obj.page}`,
        config
      );
...
export const getAllProducts = createAsyncThunk(
  ...
  async (obj, thunkAPI) => {
    try {
      const metaData = await axios.get(`/malmo/product`);
...
export const getAllCourses = createAsyncThunk(
  ...
  async (obj, thunkAPI) => {
    try {
      const metaData = await axios.get(`/malmo/courses`);
...
export const getAllRooms = createAsyncThunk(
  ...
  async (obj, thunkAPI) => {
    try {
      const metaData = await axios.get(`/malmo/rooms`);
...
export const getProductsByGender = createAsyncThunk(
  "/malmo/product/:gender",
  async (obj, thunkAPI) => {
    try {
      const metaData = await axios.get(
        `/malmo/product/${obj.gender}/${obj.age}`
      );
...
export const getProductsByCategory = createAsyncThunk(
  ...
  async (obj, thunkAPI) => {
    try {
      const metaData = await axios.get(
        `/malmo/product/${obj.gender}/${obj.category}/${obj.age}`
      );
      thunkAPI.dispatch(setFilteredProducts(metaData.data.product));
      return metaData.data.product;
    } catch (err) {
...
export const getCourseById = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      const metaData = await axios.get(`/malmo/courses/detail/${id}`);
...
export const auhtGoogle = createAsyncThunk(
  ...
  async (tokenId, thunkAPI) => {
    try {
      const metaData = await axios.post("/malmo/auth/googlelogin", {
        tokenId,
      });
      thunkAPI.dispatch(setValidateUser(metaData.data.user));
      thunkAPI.dispatch(setAuthToken(metaData.data.token));
...
export const deleteProduct = createAsyncThunk(
  ...
  async (obj, thunkAPI) => {
    try {
      ...
      const metaData = await axios.delete(
        `/malmo/productsprivate/deleteProduct`,
        config
      );
      thunkAPI.dispatch(getAllProducts());
    } catch (err) {
...
export const editImage = createAsyncThunk(
  ...
  async (obj, thunkAPI) => {
    ...
    try {
      const metaData = await axios.put(
        "/malmo/usersprivate/editImage/profile",
        { url: obj.url },
        config
      );
...
export const getProductsByName = createAsyncThunk(
  ...
  async (name, thunkAPI) => {
    try {
      const metaData = await axios.get(`/malmo/product/${name}`);
      thunkAPI.dispatch(setFilteredProducts(metaData.data.product));
      return metaData.data;
    } catch (err) {
      thunkAPI.dispatch(setFilteredProducts([]));
